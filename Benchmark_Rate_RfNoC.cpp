...
// Changes in functionality: rx_delay and tx_delay no longer effect the program, program will only send and recieve from 1 channel, random_samps function removed
    ...
    bool rx_stream_now)
{
    ...
    cmd.time_spec  = time_spec(12);
    std::cout << "Starting Recieve at" << "[" << NOW() << "]" << std::endl;
    ...
    float recv_timout = 12; //changed to make sure timeout isn't triggered while waiting for the set time to begin recieving
    ...
    bool random_nsamps = false)
{
    ...
    if (duc_ctrl) {
        ...
        std::cout << "  Interpolation value is " << duc_ctrl->get_property<int>("interp", duc_chan) << std::endl;
    ...
    std::this_thread::sleep_for(std::chrono::duration<double>(tx_delay));

    //ADDED
    //Configrue Replay
    uint32_t replay_buff_addr = 0;
    const size_t sample_size = 8; //complex floating point 32 bit is 64 bits total (only works for default cpu)
    uint32_t replay_buff_size = spb * sample_size;
    size_t replay_chan = 0;
    size_t num_samps = duration * tx_rate;
    replay_ctrl->config_play(replay_buff_addr, replay_buff_size, replay_chan);
    uhd::stream_cmd_t stream_cmd(uhd::stream_cmd_t::STREAM_MODE_NUM_SAMPS_AND_DONE);
    stream_cmd.num_samps = num_samps;
    num_tx_samps = num_samps; // CAN YOU ASSUME THIS?
    std::cout << "Issuing replay command for " << num_samps << " samps..." << std::endl;
    //Begin Replay
    stream_cmd.ti
    stream_cmd.stream_now = false;
    stream_cmd.time_spec_t = time_spec(12);
    replay_ctrl->issue_stream_cmd(stream_cmd, replay_chan);
//make sure timeout not too short for wait
...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    uhd::stream_args_t stream_args(tx_cpu, tx_cpu);
    uhd::tx_streamer::sptr tx_stream;
    uhd::tx_metadata_t tx_md;
    tx_spb = 5000000; 
    rx_spb = 5000000;
    //rx_spb = duration * rx_rate;
    size_t num_samps = 4000; //Max bytes of replay buff is about 32000
    ...
    // Configure rfnoc block
    uint32_t replay_buff_addr = 0;
    const size_t sample_size = 8; //complex floating point 32 bit is 64 bits total (only works for default cpu)
    uint32_t replay_buff_size = num_samps * uhd::convert::get_bytes_per_item(tx_cpu);
    ...
    std::cout << "Record fullness:      " << replay_ctrl->get_record_fullness(replay_chan)
         << " bytes" << std::endl
         << std::endl;
         
         
    // setup variables and allocate buffer
    std::vector<char> buff(num_samps * uhd::convert::get_bytes_per_item(tx_cpu));
    std::vector<const void*> buffs;
    for (size_t ch = 0; ch < tx_stream->get_num_channels(); ch++)
        buffs.push_back(&buff.front()); // same buffer for each channel
    ...
    tx_md.end_of_burst   = true; //missing the set up streamer and commit graph
    ...
    // send() call.
    size_t num_tx_samps_replay = tx_stream->send(buffs, num_samps, tx_md, 5.0);
    if (num_tx_samps_replay != num_samps) {
        std::cout << "ERROR: Unable to send " << num_samps << " samples (sent "
             << num_tx_samps_replay << ")" << std::endl;
        return EXIT_FAILURE;
    }
